...
The obvious `useState` + `useEffect` shape does not survive this repo's lint:
re-syncing on a changed `query` means a synchronous `setState` inside the
effect, which `react-hooks/set-state-in-effect` rejects as an error.
